...
letter_strip <- theme(strip.text = element_text(hjust = 0, vjust = 1.5), strip.background = element_blank())
...
hx <- cbind(transform_coord(lon = seq(-135, 180, by = 45), lat = rep(48, 8), proj.out = 3995), label = c("135°W", "90°W", "40°W", "0°", "45°E", "90°E", "135°W", "180°W"))
vx <-  cbind(transform_coord(lon = rep(180, 4), lat = c(60, 70, 80, 90), proj.out = 3995), label = c("60°N", "70°N", "80°N", "90°N"))

# Eye Index data
er <- read.csv("../csv/Eyeratiovalue1031R.csv")
er <- cbind(transform_coord(lon=er$Longitude, lat=er$Latitude, proj.out = 3995), er)
er$ER <- cut(er$Abundance, breaks=c(0, 0.0001, 0.25, 0.5, 0.75, 1), right=FALSE, labels=c("0", "0-0.25", "0.25-0.5", "0.5-0.75", "0.75-1"))
...
p_a488 <- reorder_layers(p)+
  geom_text(data=hx, aes(x=lon, y=lat, label=label), colour="gray30")+
  geom_text(data=vx, aes(x=lon, y=lat, label=label), colour="gray30")+
  geom_point(data=er, aes(x=lon, y=lat, colour=ER))+
  scale_colour_viridis_d()+
  labs(fill=expression("a488"~(m^-1)), colour="Eye Index", title=expression(bold("A")~~~"Total Absorption at 488 nm"))+
...
p_bb488 <- reorder_layers(p)+
  ...
  labs(fill=expression("bb488"~(m^-1)), colour="Eye Index", title=expression(bold("B")~~~"Total Backscatter at 488 nm"))+
  guides(fill = guide_colourbar(order=1, barheight=10), colour = guide_legend(order=2))+
  coord_sf(clip = 'off')+
  theme(legend.key = element_rect(fill = "gray80", colour = NA))
```
...
p_zeu <- reorder_layers(p)+
  ...
  geom_point(data=er, aes(x=lon, y=lat, colour=ER))+
  scale_color_viridis_d()+
  labs(fill="Zeu (m)", colour="Eye Index", title=expression(bold("A")~~~"Euphotic depth"))+
  guides(fill = guide_colourbar(order=1, barheight=10), colour = guide_legend(order=2))+
  coord_sf(clip = 'off')+
  theme(legend.key = element_rect(fill = "gray80", colour = NA))
```
...
p_par <- reorder_layers(p)+
  ...
  geom_point(data=er, aes(x=lon, y=lat, colour=ER))+
  scale_color_viridis_d()+
  labs(fill=expression("PAR"~(E~m^-2*d^-1)), colour="Eye Index", title=expression(bold("B")~~~"Photosynthetically available radiation"))+
  guides(fill = guide_colourbar(order=1, barheight=10), colour = guide_legend(order=2))+
  coord_sf(clip = 'off')+
  theme(legend.key = element_rect(fill = "gray80", colour = NA))
```
...
```{r, fig.width=6, fig.height=5, fig.cap="Fig. 1. Distribution map showing the location and eye index (EII) of each sample in the Arctic Ocean.  ADD Source of Basemap."}
...
reorder_layers(p)+
  ...
  geom_point(data=er, aes(x=lon, y=lat, colour=ER))+
  scale_color_viridis_d()+
  labs(fill="Depth (m)", colour="Eye Index")+
  coord_sf(clip = 'off')
```
...
p_tvis0 <- reorder_layers(p)+
  ...
  geom_point(data=er, aes(x=lon, y=lat, colour=ER))+
  scale_color_viridis_d()+
  labs(fill=expression(italic(T[vis](0))), colour="Eye Index", title=expression(bold("A")~~~"Vertical transmittance at surface"))+
  guides(fill = guide_colourbar(order=1, barheight=10), colour = guide_legend(order=2))+
  coord_sf(clip = 'off')+
  theme(legend.key = element_rect(fill = "gray80", colour = NA))
```
...
p_tvis25 <- reorder_layers(p)+
  ...
  geom_point(data=er, aes(x=lon, y=lat, colour=ER))+
  scale_color_viridis_d()+
  labs(fill=expression(italic(T[vis](25))), colour="Eye Index", title=expression(bold("B")~~~"Vertical transmittance at 25-m"))+
  guides(fill = guide_colourbar(order=1, barheight=10), colour = guide_legend(order=2))+
  coord_sf(clip = 'off')+
  theme(legend.key = element_rect(fill = "gray80", colour = NA))
```
...
p_par25 <- reorder_layers(p)+
  ...
  geom_point(data=er, aes(x=lon, y=lat, colour=ER))+
  scale_color_viridis_d()+
  labs(fill=expression("PAR"~(25)~("E"~m^-2*d^-1)), colour="Eye Index", title=expression(bold("C")~~~"Photosynthetically available radiation at 25-m"))+
  guides(fill = guide_colourbar(order=1, barheight=10), colour = guide_legend(order=2))+
  coord_sf(clip = 'off')+
  theme(legend.key = element_rect(fill = "gray80", colour = NA))
```
...
p_tvis_zeu <- reorder_layers(p)+
  ...
  geom_point(data=er, aes(x=lon, y=lat, colour=ER))+
  scale_color_viridis_d()+
  labs(fill=expression(italic(T[vis](Z[eu]))), colour="Eye Index", title=expression(bold("D")~~~"Vertical transmittance at Zeu"))+
  guides(fill = guide_colourbar(order=1, barheight=10), colour = guide_legend(order=2))+
  coord_sf(clip = 'off')+
  theme(legend.key = element_rect(fill = "gray80", colour = NA))
```
...
p_tvis_bottom <- reorder_layers(p)+
  ...
  labs(fill=expression(italic(T[vis](z))), colour="Eye Index", title=expression(bold("A")~~~"Vertical transmittance at bottom"))+
  guides(fill = guide_colourbar(order=1, barheight=10), colour = guide_legend(order=2))+
  coord_sf(clip = 'off')+
  theme(legend.key = element_rect(fill = "gray80", colour = NA))
```
...
p_par_bottom <- reorder_layers(p)+
  ...
  geom_point(data=er, aes(x=lon, y=lat, colour=ER))+
  scale_color_brewer(type="seq", palette="YlOrBr", direction=-1)+
  labs(fill=expression("PAR"~(italic(z))~("E"~m^-2*d^-1)), colour="Eye Index", title=expression(bold("B")~~~"Photosynthetically available radiation at bottom"))+
  guides(fill = guide_colourbar(order=1, barheight=10), colour = guide_legend(order=2))+
  coord_sf(clip = 'off')+
  theme(legend.key = element_rect(fill = "gray80", colour = NA))
```
...
write.csv(out, file= "../csv/Eyeratiovalue1031R_Tvis.csv", row.names=FALSE)
```
...
s <- lapply(splitBy(~variable, ml0)[-3], FUN=function(x)glmer(Eye_Ratio~scale(Water.Depth) + (1 | Region), family = binomial, data=x) %>% summary)
...
s2 <- lapply(splitBy(~variable, ml0)[-3], FUN=function(x){
  y <- cbind(x$withEye, x$noEye)
  glmer(y~scale(Water.Depth) + (1 | Region), family = binomial, data=x) %>% summary
  })

lapply(s2, FUN=function(x)cbind(Variable=rownames(x$coef), data.frame(x$coef)))%>%ldply%>%kable(caption="Table x. GLM with binomial distribution on the numbers of specimen with eyes and without eye with water depth as fixed factor and regions as random factor") %>% kable_classic(full_width = F, html_font = "Cambria", font_size=16)

ml0$Lab <- factor(ml0$variable, labels = c("A   Individual", "C   Species", NA))
...
iso <- data.frame(Water.Depth, Eye_Ratio, variable = "Species", Lab = "C   Species")

p_depth_eye1 <- ggplot(data=subset(ml0, variable!="Genus"), aes(x=Water.Depth, y=Eye_Ratio))+
  geom_point(aes(colour=Region))+
  stat_smooth(data=subset(ml0, variable!="Genus"), method="glm", formula = y~x, method.args = list(family = "binomial"))+
...
```{r}
ml0$Lab <- factor(ml0$variable, labels = c("B   Individual", "D   Species", NA))
iso$Lab <- "D   Species"

p_depth_eye2 <- ggplot(data=subset(ml0, variable!="Genus"), aes(x=Water.Depth, y=Eye_Ratio))+
  geom_point(aes(colour=Region))+
...
```{r, fig.width=10, fig.height=8, fig.cap="Fig. 3. Bathymetric distribution of eye index value (EII, EIS) in Arctic Ocean. Panel (A), (B) are computed by abundance method; Panel (C), (D) are computed by species method; Panel (C), (D), only show the sample from shallow region (0-500m). Red symbols and dashed lines show isopod eye index (EIS) from Menzies et al. (1968)."}
p1 <- p_depth_eye1+labs(y="Eye Index") + no_legend
p2 <- p_depth_eye2+theme(axis.text.y = element_blank(), axis.title.y = element_blank()) 
p1+p2
```

```{r, fig.width=10, fig.height=8, fig.cap="Figure 4. Bathymetric distribution of eye index values computed by abundance method (EII) in different regions (A–F) of the Arctic Ocean. Only samples from relatively shallow regions (0-500 m) are presented to show the regional difference of eye index in shallow water among the various Arctic Ocean coastal seas."}
eii_r <- subset(ml0, variable =="Individual")
...
```{r, eval=FALSE}
s <- lapply(splitBy(~variable, ml)[-3], FUN=function(x)glmer(Eye_Ratio~Tvis+(1|Region), family = binomial, data=x) %>% summary)

lapply(s, FUN=function(x)cbind(Variable=rownames(x$coef), data.frame(x$coef)))%>%ldply%>%kable(caption="Table x. GLM with binomial distribution on eye indices with water depth as fixed factor and regions as random factor") %>% kable_classic(full_width = F, html_font = "Cambria", font_size=16)

ml$Lab <- factor(ml$variable, labels = c("A   Individual", "B   Species", NA))

ggplot(data=subset(ml, variable!="Genus"), aes(x=Tvis, y=Eye_Ratio))+
  geom_point(aes(colour=Region))+
...
```{r}
s <- lapply(splitBy(~variable, ml0)[-3], FUN=function(x)glmer(Eye_Ratio~Tvis_buffer+(1|Region), family = binomial, data=x) %>% summary)
lapply(s, FUN=function(x)cbind(Variable=rownames(x$coef), data.frame(x$coef)))%>%ldply%>%kable(caption="Table x. GLM with binomial distribution on eye indices with water depth as fixed factor and regions as random factor") %>% kable_classic(full_width = F, html_font = "Cambria", font_size=16)

ml0$Lab <- factor(ml0$variable, labels = c("A   Individual", "B   Species", NA))

p_Tvis_eye <- ggplot(data=subset(ml0, variable!="Genus"), aes(x=Tvis_buffer, y=Eye_Ratio))+
  geom_point(aes(colour=Region))+
...
```{r, eval=FALSE}
s <- lapply(splitBy(~variable, ml)[-3], FUN=function(x)glmer(Eye_Ratio~Tvis_PAR+(1|Region), family = binomial, data=x) %>% summary)
...
ggplot(data=subset(ml, variable!="Genus"), aes(x=Tvis*PAR*6.97E12, y=Eye_Ratio))+
  geom_point(aes(colour=Region))+
...
```{r}
s <- lapply(splitBy(~variable, ml0)[-3], FUN=function(x)glm(Eye_Ratio~Tvis_PAR, family = binomial, data=x) %>% summary)

lapply(s, FUN=function(x)cbind(Variable=rownames(x$coef), data.frame(x$coef)))%>%ldply%>%kable(caption="Table x. GLM with binomial distribution on eye indices with water depth as fixed factor and regions as random factor") %>% kable_classic(full_width = F, html_font = "Cambria", font_size=16)

ml0$Lab <- factor(ml0$variable, labels = c("C   Individual", "D   Species", NA))

# E m-2 d-1 = 6.97 x 10^12 μmol quanta m-2 s-1
p_PAR_eye <- ggplot(data=subset(ml0, variable!="Genus"), aes(x=Tvis_PAR*6.97E12, y=Eye_Ratio))+
  geom_point(aes(colour=Region))+
  stat_smooth(data=subset(ml0, variable!="Genus"), method="glm", formula = y~x, family = binomial, method.args = list(family = "binomial"))+
  coord_flip()+
  scale_x_log10()+
  scale_colour_viridis_d()+
...
```{r, fig.width=10, fig.height=8, fig.cap="Fig. 6. Eye index (EII and EIS) as functions of the vertical transmittance (Tvis) and photosynthetically available radiation (PAR) at sampling depth. Panel (A), (C) are computed by abundance method; Panel (B), (D) are computed by species method."}
...
```{r, fig.width=10, fig.height=4, fig.cap="Fig. 7 Latitudinal gradient of eye index (EII and EIS). Panel (A) is computed by abundance method; Panel (B) is computed by species method."}
s <- lapply(splitBy(~variable, ml0)[-3], FUN=function(x)glmer(Eye_Ratio~Latitude+(1|Region), family = binomial, data=x) %>% summary)

lapply(s, FUN=function(x)cbind(Variable=rownames(x$coef), data.frame(x$coef)))%>%ldply%>%kable(caption="Table x. GLM with binomial distribution on eye indices with water depth as fixed factor and regions as random factor") %>% kable_classic(full_width = F, html_font = "Cambria", font_size=16)

ml0$Lab <- factor(ml0$variable, labels = c("A   Individual", "B   Species", NA))

ggplot(data=subset(ml0, variable!="Genus"), aes(x=Latitude, y=Eye_Ratio))+
  geom_point(aes(colour=Region))+
  stat_smooth(method="glm", formula = y~x, method.args = list(family = "binomial"))+
  scale_colour_viridis_d()+
...
s <- lapply(splitBy(~variable, ml0)[-3], FUN=function(x)glmer(Eye_Ratio~Water.Depth + Latitude + (1 | Region), family = binomial, data=x) %>% summary)
...
```{r, fig.width=10, fig.height=8, fig.cap="Fig. x. Eye index (EII and EIS) as functions of the sampling depth and latitude. Panel (A), (C) are computed by abundance method; Panel (B), (D) are computed by species method."}

ml0$Lab <- factor(ml0$variable, labels = c("A   Individual", "B   Species", NA))

p1 <- ggplot(data=subset(ml0, variable!="Genus"), aes(x=Water.Depth, y=Eye_Ratio))+
  geom_point(aes(colour=Region))+
  stat_smooth(data=subset(ml0, variable=="Species"), method="glm", formula = y~x, method.args = list(family = "binomial"))+
  coord_flip()+
  scale_x_reverse()+
...
p2 <- ggplot(data=subset(ml0, variable!="Genus"), aes(x=Latitude, y=Eye_Ratio))+
  geom_point(aes(colour=Region))+
  stat_smooth(data=subset(ml0, variable=="Individual"), method="glm", formula = y~x, method.args = list(family = "binomial"))+
  scale_colour_viridis_d()+
  labs(x=expression("Latitude"~(degree*N)), y="Eye Index")+
  facet_wrap(~Lab, scales="free")+
  theme_bw() %+replace% large %+replace% letter_strip
...
s <- lapply(splitBy(~variable, subset(ml0, Tvis_PAR>0))[-3], FUN=function(x)glmer(Eye_Ratio~Tvis_PAR + Latitude + (1 | Region), family = binomial, data=x) %>% summary)
...
```{r, fig.width=10, fig.height=8, fig.cap="Fig. x. Eye index (EII and EIS) as functions of the photosynthetically available radiation (PAR) at sampling depth and latitude. Panel (A), (C) are computed by abundance method; Panel (B), (D) are computed by species method."}

ml0$Lab <- factor(ml0$variable, labels = c("A   Individual", "B   Species", NA))

# E m-2 d-1 = 6.97 x 10^12 μmol quanta m-2 s-1
p1 <- ggplot(data=subset(ml0, variable!="Genus"), aes(x=Tvis_PAR*6.97E12, y=Eye_Ratio))+
  geom_point(aes(colour=Region))+
  stat_smooth(data=subset(ml0, variable=="Species"), method="glm", formula = y~x, method.args = list(family = "binomial"))+
  coord_flip()+
  scale_x_log10()+
...
```{r}
summary(glmer(Tvis_buffer~Water.Depth+(1|Region), family = gaussian(link = "log"), data=subset(out, Tvis_buffer>0.001)))$coef %>% kable(caption = "Table x. GLM with gaussian distribution and logarithmic link function on Tvis. Data with Tvis < 0.1% were deleted before analysis.") %>% kable_classic(full_width = F, html_font = "Cambria", font_size=16)
...
summary(glmer(Tvis_PAR~Water.Depth+(1|Region), family = gaussian(link = "log"), data=subset(out, Tvis_buffer>0.001)))$coef%>% kable(caption = "Table x. GLM with gaussian distribution and logarithmic link function on PAR(z) at sampling depth. Data with Tvis < 0.1% were deleted before analysis.") %>% kable_classic(full_width = F, html_font = "Cambria", font_size=16)
...
```{r, fig.width=12, fig.height=6, fig.cap="Fig. X. Vertical transmittance (Tvis) and photosynthetically available radiation (PAR) at sampling depth as functions of water depth. Data with Tvis = 0% were removed before conducting GLM with with gaussian distribution and logarithmic link"}
...
```{r, fig.width=12, fig.height=6, fig.cap="Fig. 8. Vertical transmittance (Tvis) and photosynthetically available radiation (PAR) at sampling depth as functions of water depth. Data with Tvis = 0% were removed before conducting GLM with with gaussian distribution and logarithmic link"}
p1_log <- p1+scale_y_log10() + stat_smooth(method="glm", formula = y~x)
p2_log <- p2+scale_y_log10() + stat_smooth(method="glm", formula = y~x)
p1_log+p2_log
```
...
```{r, fig.width=12, fig.height=10, fig.cap="Fig. 8 Vertical transmittance and Photosynthetically available radiation at sampling depth [Tvis (z) and PAR (z), respectively] as functions of water depth. (A–B) for the top 70 m; (C–D) for full bathymetric range. Data with Tvis (z) less than 0.1% were removed before conducting GLM with gaussian distribution and logarithmic link function."}
p1 <- p_Tvis_depth + labs(y=expression(italic(T[vis](z)))) + no_legend
p2 <- p_PAR_depth + labs(y=expression("PAR"~(italic(z))~(mu*mol~"quanta"~m^-2*s^-1))) 
p3 <- p1_log + labs(tag="C")
p4 <- p2_log + labs(tag="D") + no_legend
