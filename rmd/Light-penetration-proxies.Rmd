...
```{r, eval=FALSE, fig.width=12, fig.height=5, fig.cap="Fig.S1. IOP total absorption at 488 nm and total backscatter at 488 nm based on MODIS_Aqua entire mission composite climatology."}
p_a488+p_bb488
```
...
```{r}
s <- lapply(splitBy(~variable, ml0)[-3], FUN=function(x)glm(Eye_Ratio~Water.Depth, family = binomial, data=x) %>% summary)
lapply(s, FUN=function(x)cbind(Variable=rownames(x$coef), data.frame(x$coef)))%>%ldply%>%kable(caption="Table x. GLM with binomial distribution on eye indices")

ml0$Lab <- factor(ml0$variable, labels = c("A   Abundance", "C   Species", NA))
...
```{r}
ml0$Lab <- factor(ml0$variable, labels = c("B   Abundance", "D   Species", NA))
iso$Lab <- "D   Species"
...
```{r, eval=FALSE}
s <- lapply(splitBy(~variable, ml)[-3], FUN=function(x)glm(Eye_Ratio~Tvis, family = binomial, data=x) %>% summary)
lapply(s, FUN=function(x)cbind(Variable=rownames(x$coef), data.frame(x$coef)))%>%ldply%>%kable(caption="Table x. GLM with binomial distribution on eye indices")

ml$Lab <- factor(ml$variable, labels = c("A   Abundance", "B   Species", NA))

ggplot(data=subset(ml, variable!="Genus"), aes(x=Tvis, y=Eye_Ratio))+
  geom_point(aes(colour=Region))+
  stat_smooth(data=subset(ml, variable=="Individual"), method="glm", formula = y~x, method.args = list(family = "binomial"))+
  scale_colour_viridis_d()+
...
```{r}
s <- lapply(splitBy(~variable, ml0)[-3], FUN=function(x)glm(Eye_Ratio~Tvis_buffer, family = binomial, data=x) %>% summary)
lapply(s, FUN=function(x)cbind(Variable=rownames(x$coef), data.frame(x$coef)))%>%ldply%>%kable(caption="Table x. GLM with binomial distribution on eye indices")

ml0$Lab <- factor(ml0$variable, labels = c("A   Abundance", "B   Species", NA))
...
```{r, eval=FALSE}
s <- lapply(splitBy(~variable, ml)[-3], FUN=function(x)glm(Eye_Ratio~Tvis_PAR, family = binomial, data=x) %>% summary)
lapply(s, FUN=function(x)cbind(Variable=rownames(x$coef), data.frame(x$coef)))%>%ldply%>%kable(caption="Table x. GLM with binomial distribution on eye indices")

ml$Lab <- factor(ml$variable, labels = c("A   Abundance", "B   Species", NA))

ggplot(data=subset(ml, variable!="Genus"), aes(x=Tvis*PAR, y=Eye_Ratio))+
  geom_point(aes(colour=Region))+
  stat_smooth(data=subset(ml, variable=="Individual"), method="glm", formula = y~x, family = binomial, method.args = list(family = "binomial"))+
  scale_colour_viridis_d()+
  labs(x=expression("Photosynthetically available radiation"~(E~m^-2*d^-1)~"at sampling depth"), y="Eye Index")+
  facet_wrap(~Lab, scales="free")+
...
ml0$Lab <- factor(ml0$variable, labels = c("C   Abundance", "D   Species", NA))

p_PAR_eye <- ggplot(data=subset(ml0, variable!="Genus"), aes(x=Tvis_PAR, y=Eye_Ratio))+
  geom_point(aes(colour=Region))+
  stat_smooth(data=subset(ml0, variable!="Genus"), method="glm", formula = y~x, family = binomial, method.args = list(family = "binomial"))+
  scale_colour_viridis_d()+
  labs(x="Photosynthetically available radiation (einstein m^-2 day^-1) at sampling depth with 100-km Buffer", y="Eye Index")+
  facet_wrap(~Lab, scales="free")+
  theme_bw() %+replace% large %+replace% letter_strip
```
...
p2 <- p_PAR_eye+labs(x=expression("PAR"~(italic(z))~(E~m^-2*d^-1)), y="Eye Index") + no_legend
p1/p2
```
...
s <- lapply(splitBy(~variable, ml0)[-3], FUN=function(x)glm(Eye_Ratio~Latitude, family = binomial, data=x) %>% summary)
lapply(s, FUN=function(x)cbind(Variable=rownames(x$coef), data.frame(x$coef)))%>%ldply%>%kable(caption="Table x. GLM with binomial distribution on eye indices")
...
```{r}
summary(glm(Tvis_buffer~Water.Depth, family = gaussian(link = "log"), data=subset(out, Tvis_buffer>0.001)))$coef %>% kable(caption = "Table x. GLM with gaussian distribution and logarithmic link function on Tvis. Data with Tvis < 0.1% were deleted before analysis.")
...
summary(glm(Tvis_PAR~Water.Depth, family = gaussian(link = "log"), data=subset(out, Tvis_buffer>0.001)))$coef%>% kable(caption = "Table x. GLM with gaussian distribution and logarithmic link function on PAR(z) at sampling depth. Data with Tvis < 0.1% were deleted before analysis.")

p_PAR_depth <- ggplot(data=subset(out, Tvis_buffer!=0), aes(x=Water.Depth, y=Tvis_PAR))+
  geom_point(aes(colour=Region))+
  stat_smooth(method="glm", formula = y~x, method.args = list(family = gaussian(link = "log")))+
  scale_colour_viridis_d()+
  labs(x=expression("Water depth"~(m)), y=expression("PAR"~(italic(z))~(E~m^-2*d^-1)), tag="B")+
  theme_bw() %+replace% large %+replace% vert_x 
```
...
p2 <- p_PAR_depth + labs(y=expression("PAR"~(italic(z))~(E~m^-2*d^-1))) 
p1+p2
```
...
p_PAR_depth <- ggplot(data=subset(out, Tvis_buffer>0.001), aes(x=Water.Depth, y=Tvis_PAR))+
  geom_point(aes(colour=Region))+
  stat_smooth(method="glm", formula = y~x, method.args = list(family = gaussian(link = "log")))+
  scale_colour_viridis_d()+
  labs(x=expression("Water depth"~(m)), y=expression("PAR"~(italic(z))~(E~m^-2*d^-1)), tag="B")+
